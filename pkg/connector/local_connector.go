...
func (c *localConnector) getHostInfo(ctx context.Context) (map[string]any, error) {
	...
	case "linux":
		...
		osVars[_const.VariableOSKernelVersion] = string(bytes.TrimSpace(bytes.Split(buf.Bytes(), []byte(" "))[2]))
		matches := regexp.MustCompile(`(x86_64|aarch64|arm64|amd64)`).FindStringSubmatch(buf.String())
		if len(matches) == 0 {
